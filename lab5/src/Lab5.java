...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	private void corkscrew(GL2 gl2) {
		...
		for (int i = 0; i < amountOfPoints; i++)
		{
			...
			if(i < amountOfPoints / 3){
				gl2.glColor3f(0,1,0);
			}
			else if(i < amountOfPoints * 2 / 3){
				gl2.glColor3f(.588f, .294f, 0f);
			} else {
				gl2.glColor3f(0,0,1);
			}

			float v0 = (float)(3.5 * Math.cos(i * 2 * Math.PI / (amountOfPoints / 12)));
			float v1 = (float)(3.5 * Math.sin(i * 2 * Math.PI / (amountOfPoints / 12)));
			gl2.glVertex3f(v0, v1,0.01f * i * -1);
			gl2.glEnd();
		}
	}
